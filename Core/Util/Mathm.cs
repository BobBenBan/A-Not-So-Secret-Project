...
{
    public static int Lerpi(int from, int to, float step) =>
        (from + (to - from) * step).RoundToInt();

    public static byte Lerpb(byte from, byte to, float step) =>
        (from + (to - from) * step).RoundToByte();
