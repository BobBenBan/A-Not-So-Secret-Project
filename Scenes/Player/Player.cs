...
public class Player : KinematicBody
{
    ...
    private void ProcessMovement(float delta)
    {
        ...
        if (Enabled)
        {
            ...
            if (CanFly &&
                Inputs.PlayerDoubleJump.DoubleTapped(DoubleTapTime, delta))
                _flying = !_flying;

            if (isOnFloor) _flying = false;
            //speed
            var slow = slowPressed && !_flying;
            _running = !slow && runningPressed &&
                       (_running || _flying || isOnFloor);
        ...
        var accel = (targetVel - actualVel).Dot(actualVel) > 0
            ? Acceleration
            : Deceleration;
        ...
        _outputVel = MoveAndSlide(actualVel, _up, true, 2,
